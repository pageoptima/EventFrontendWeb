...
import { getCountdownLabel, getEventDateLabel } from "@/utils/helpers";
  ...
  return (
    <article className="space-y-3">
      <header className="flex items-center justify-between">
        ...
        <button
          type="button"
          className="text-xs font-semibold text-[#B839F1] transition hover:text-[#FF2727]"
        >
          Follow
        </button>
      </header>
      <div className="relative w-[228px] overflow-hidden rounded-[10px] border border-border bg-muted">
        <img
          src={event.image}
          alt={`${event.user.username} event`}
          className="h-[169px] w-[228px] object-cover"
          loading="lazy"
        />
