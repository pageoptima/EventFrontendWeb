...
import { useDeleteTeaser, useChangeTeaserVisibility } from "@/features/teaser/hooks/useTeaser";

const VISIBILITY_OPTIONS = [
  {
    value: "PUBLIC",
    label: "Public",
    description: "Anyone can see this teaser",
    Icon: Globe,
  },
  {
    value: "FRIENDS",
    label: "Friends",
    description: "Only your friends can see this teaser",
    Icon: Users,
  },
  {
    value: "PRIVATE",
    label: "Private",
    description: "Only you can see this teaser",
    Icon: Lock,
  },
];
  ...
  return (
    <>
      <div className="border-b border-border px-4 py-3 text-center">
        <p className="text-sm font-semibold text-foreground">Teaser options</p>
      </div>
      <button
        type="button"
...
function VisibilityView({ currentVisibility, teaserId, onBack, onClose }) {
  const [selected, setSelected] = useState(currentVisibility ?? "PUBLIC");
  const { mutate, isPending } = useChangeTeaserVisibility(teaserId);
...
function DeleteConfirmView({ teaserId, onBack, onClose }) {
  const { mutate, isPending } = useDeleteTeaser(teaserId);

  return (
    <>
      <div className="border-b border-border px-4 py-3 text-center">
        <p className="text-sm font-semibold text-foreground">Delete teaser?</p>
      </div>
...
function TeaserOptionsMenu({ teaserId, currentVisibility }) {
...
export default TeaserOptionsMenu;
